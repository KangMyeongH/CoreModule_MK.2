...
void engine::MeshRenderer::InputAssembler(ID3D11DeviceContext* context)
{
	if (m_Mesh)
	{
...
void engine::MeshRenderer::Bind(ID3D11DeviceContext* context)
{
	...
	for (const auto& materialPair : m_Material)
	{
		if (materialPair.second->GetShader())
		{
			const auto& mat = materialPair.second;
			mat->SetSampler("Sampler", sampler);
			mat->SetColor("DirLight_Ambient", _float4(1.f, 1.f, 1.f, 1.f));
			mat->SetColor("DirLight_Specular", _float4(1.f, 1.f, 1.f, 1.f));
			mat->SetMatrix("g_WorldMatrix", GetTransform()->GetWorldMatrix());
		}
	}
}

void engine::MeshRenderer::Render(ID3D11DeviceContext* context)
{
...
void engine::MeshRenderer::PreRender(ID3D11DeviceContext* context, const _float4X4& viewMat, const _float4X4& projMat)
{
